...
export default function PrayerListScreen() {
  ...
  const { state, hasSentPrayer } = useAppContext();
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.listIntro}>
          <Text style={styles.listIntroText}>
            Browse prayer requests from believers worldwide. Each person has shared their need in faith, trusting the community to intercede.
          </Text>
        </View>
        ...
        {state.selectionMode === 'auto' ? (
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <View style={styles.autoCtaLeft}>
              <View style={styles.autoIcon}>
                <Zap size={20} color="#FFF" fill="#FFF" />
              </View>
              <Text style={styles.autoCtaText}>Pray Where Needed Most (Auto)</Text>
            </View>
        ...
        {showFilters && (
          <View style={styles.filtersPanel}>
            <TouchableOpacity style={styles.filterSelect} onPress={() => setShowCountryPicker(true)}>
              <Text style={countryFilter ? styles.filterSelectText : styles.filterSelectPlaceholder}>
                {countryFilter ? UNIQUE_COUNTRIES.find(c => c.code === countryFilter)?.name : 'All Countries'}
              </Text>
              <ChevronDown size={18} color="#6B4F3E" />
            </TouchableOpacity>

            <TouchableOpacity style={styles.filterSelect} onPress={() => setShowDenominationPicker(true)}>
              <Text style={denominationFilter ? styles.filterSelectText : styles.filterSelectPlaceholder}>
            ...
            {(countryFilter || denominationFilter) && (
              <GhostButton size="sm" onPress={clearFilters}>Clear filters</GhostButton>
            )}
          </View>
        )}

        <Text style={styles.resultsCount}>
          {filteredRequests.length} requests {(countryFilter || denominationFilter || searchQuery) && '(filtered)'}
        </Text>
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>
                {hasSentPrayer ? "You've prayed for all available requests" : 'No requests match your search'}
              </Text>
              <Text style={styles.emptySubtitle}>Check back later for new prayer requests</Text>
            </View>
          }
        />
      </View>
      ...
      {renderPicker(
        ...
        'Select Denomination',
        DENOMINATIONS.filter(d => d !== 'Prefer not to say').map((d) => ({ label: d, value: d })),
        denominationFilter,
        setDenominationFilter
      )}
    </View>
